...
cc::pair<pr::auto_shader_binary, phi::detail::unique_buffer> load_shader(pr::Context& ctx, char const* path, phi::shader_stage stage, char const* path_prefix = "");

/// loads a .obj or binary mesh from disk and flushes GPU completely, result is usable immediately
pr_mesh load_mesh(pr::Context& ctx, char const* path, bool binary = false);
}
